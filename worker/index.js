...
Google-таблицы (plan_deltas — положительное число значит "ещё нужно докупить на эту сумму",
отрицательное значит "актив в избытке, продать этот модуль суммы", ноль/отсутствие значит "по плану
всё в порядке") и таблицу плановых траншей (plan_rows) со статусом каждого. У каждого транша есть
поле side: "buy" (докупка на просадке) или "sell" (продажа — например, плановое избавление от актива
к определённому сроку, без привязки к просадке), и уже готовое поле effective_amount — это ИТОГОВАЯ
сумма к покупке/продаже для этого транша (посчитана заранее в коде: amount_plan, если задан, иначе
живой модуль из plan_deltas). Это число уже финальное — не пересчитывай его сам и не выбирай между
amount_plan/plan_deltas самостоятельно, просто используй effective_amount как есть. Если
effective_amount равен null — значит сумма нигде не определена, так и скажи, не придумывай число.
...
5. Сумма к покупке/продаже для сработавшего транша — это его effective_amount, без исключений.
...
export default {
  async fetch(request, env) {
    ...
    // выберет между amount_plan/plan_deltas или сверит два списка):
    // 1) итоговую сумму на транш (effective_amount) — override amount_plan,
    //    иначе живой модуль дельты план/факт по активу;
    // 2) какие активы из plan_deltas НЕ покрыты ни одним траншем в plan_rows —
    //    по ним нужно правило 8 (явное "ПРЕДЛОЖЕНИЕ" по тайнингу, не молчание).
    const effectiveAmount = (row) => {
      if (row.amount_plan !== null && row.amount_plan !== undefined && row.amount_plan !== "" && row.amount_plan !== 0) {
        return Math.abs(row.amount_plan);
      }
      const delta = planDeltas[row.asset];
      return delta !== null && delta !== undefined ? Math.abs(delta) : null;
    };
    const planRowsWithAmount = planRows.map((r) => ({ ...r, effective_amount: effectiveAmount(r) }));
    ...
    const userMessage =
      `Текущие данные плана (JSON):\n\n${JSON.stringify({ asset_prices: assetPrices, plan_deltas: planDeltas, plan_rows: planRowsWithAmount, today }, null, 2)}\n\n` +
